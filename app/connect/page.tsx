...
export default function ConnectPage() {
  return (
    <>
      <Hero eyebrow="Connect" title={<>Prayer, visit info, and <em>socials</em>.</>} copy="Submit a prayer request, plan your visit to Kangundo Road, or follow the active SOA social platforms." image={heroImages.worship} primary={{ label: "Send Prayer Request", href: "#prayer" }} />
      ...
      <section className="section panel">
        <div className="container">
          ...
          <div className="insta-fallback">{socials.map(({ name, handle, url, color, mark }) => <a key={name} href={url} target="_blank" rel="noreferrer" style={{ "--social": color } as React.CSSProperties}><b>{mark}</b><span>{name}</span><small>{handle}</small></a>)}</div>
        </div>
      </section>
